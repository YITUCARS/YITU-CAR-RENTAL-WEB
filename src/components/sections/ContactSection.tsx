...
export default function ContactSection() {
  ...
  const features = [
    {icon: Bell, label: t('ContactSection.features.notifications')},
    {icon: Headphones, label: t('ContactSection.features.bilingualSupport')},
    {icon: CalendarCheck, label: t('ContactSection.features.confirmation')},
    {icon: MapPin, label: t('ContactSection.features.airportPickup')},
  ]
      ...
      <div className="max-w-[1100px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-[60px] items-center">
        <div>
          ...
          <h2 className="font-montserrat font-extrabold italic text-[clamp(1.8rem,3vw,2.6rem)] text-navy leading-[1.15] mb-4">
            {t('ContactSection.titlePrefix')} <span className="text-orange">{t('ContactSection.titleAccent')}</span>
          </h2>
          ...
          <div className="relative z-10 w-[252px] bg-white border border-black/10 rounded-[34px] overflow-hidden shadow-[0_26px_60px_rgba(15,23,42,0.18)]">
            ...
            <div className="px-2.5 pb-2.5">
              ...
              <div className="bg-off-white rounded-[14px] p-3 mb-2">
                ...
                <div className="font-syne font-bold text-[11px] text-navy mt-2.5">{t('ContactSection.scanToChat')}</div>
                <div className="text-[10px] text-muted mt-0.5">{t('ContactSection.scanDescription')}</div>
                <div className="flex items-center justify-between mt-2.5">
                  ...
                  <div className="bg-[#07c160] text-white text-[10px] font-syne font-bold px-3 py-1.5 rounded-[6px]">
                    {t('ContactSection.addNow')}
                  </div>
                </div>
              </div>

              <div className="bg-off-white rounded-[10px] p-3">
                <div className="text-[10px] text-navy font-bold mb-1.5">{t('ContactSection.wechatContact')}</div>
                <div className="rounded-[8px] bg-white px-2.5 py-2 mb-2">
                  <div className="text-[8px] text-muted uppercase tracking-[0.18em] mb-0.5">{t('ContactSection.wechatId')}</div>
                  <div className="font-syne font-bold text-[11px] text-navy break-all">{WECHAT_ID}</div>
                </div>
                <div className="grid grid-cols-2 gap-1">
                  {[
                    ['💬', t('ContactSection.wechatFeatures.chineseSupport')],
                    ['⚡', t('ContactSection.wechatFeatures.quickReply')],
                    ['📅', t('ContactSection.wechatFeatures.bookingHelp')],
                    ['💳', t('ContactSection.wechatFeatures.paymentSupport')],
                  ].map(([emoji, label]) => (
